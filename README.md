...
**不绑任何一家模型。** 设一个环境变量指向你自己的 agent：

```sh
SB_AGENT='claude -p'        node server.js
SB_AGENT='python my_bot.py' node server.js
```

页面上就会出现「叫 AI 来看」。按下去之后，服务端起你那条命令，
把一段说明写进它的 stdin，并给三个环境变量：

```
SB_PAGE_ID    这一页的 id
SB_PAGE_FILE  页面文件的绝对路径
SB_BASE       服务地址，比如 http://localhost:4321
```

说明里告诉它：怎么读批注（`GET /api/notes?id=…`）、
每条的 `anchor` 是"她贴在哪一块内容旁边"、
怎么回话（`POST /api/notes`，`anchor` 写 `note:<id>` 就是回那一条）、
以及**它可以直接重写整个页面文件**——读者贴的东西存在别处，不会被覆盖。

一个五行的 shell 脚本就能当 agent（仓库里那条测试就是这么验的）：

```sh
#!/bin/sh
curl -s "$SB_BASE/api/notes?id=$SB_PAGE_ID"      # 看她贴了什么
curl -s -X POST "$SB_BASE/api/notes" -H 'Content-Type: application/json' \
  --data "{\"id\":\"$SB_PAGE_ID\",\"by\":\"ai\",\"kind\":\"note\",
           \"text\":\"…\",\"anchor\":\"p1\"}"
```

**不设 `SB_AGENT` 就是一本纯人用的手帐**，那个按钮不会出现——
不给按了没反应的东西。

### 一条经验：别让它定时空转

按钮是「她按了我才来」。如果你还想让 AI **趁人不在自己过来贴**，
判据要用**"有没有新东西"**，不是"隔多久"——
不然人不常做手帐时，它会一遍遍读同样的老页，每次烧一轮额度。
记一个水位线（这一页我上次回到哪儿），只在 `她动过 > 我回过` 时才跑。

## 贴纸库要自己接

`CFG.stickers` 默认是空的，**不带任何贴纸**。想要的话指向一个 GET：

```json
{ "list": [ { "url": "/stickers/cat.png", "tag": "猫咪/亲亲/抱住亲亲" } ] }
```

`tag` 是人写的描述。贴的时候会**跟着一起存**——这样读这一页的人或 AI
不用看图就知道贴的是什么，也不用事后再回库里查（远程库可能变、可能连不上）。
